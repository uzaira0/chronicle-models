...
public data class ParticipantDataPurgeSummary(
    ...
    /** Version-neutral counts keyed by the canonical data-asset ID. */
    val assetCounts: Map<String, Long> = emptyMap(),
    val totalRows: Long = if (assetCounts.isNotEmpty()) {
        assetCounts.values.sum()
    } else {
        usageEventsCount + usageStatsCount + preprocessedEventsCount + sensorDataCount +
